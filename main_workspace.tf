...
variable "instance_type" {
  description = "EC2 instance type per workspace (dev, stage, prod)"
  type        = map(string)
  default = {
    dev   = "t3.micro"
    stage = "t3.micro"
...
locals {
  workspace_instance_type = lookup(var.instance_type, terraform.workspace, "t3.micro")
}
...
module "ec2_instance" {
  ...
  region        = var.region
  ami           = var.ami
  instance_type = local.workspace_instance_type
